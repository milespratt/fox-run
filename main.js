...
import footstepWav from "./footstep.wav";
import ambientWav from "./ambient.wav";

PIXI.Loader.shared.add("footstep", footstepWav);
PIXI.Loader.shared.add("ambient", ambientWav);

PIXI.Loader.shared.load(function (loader, resources) {
  console.log(PIXI.Loader.shared.resources);
  resources.footstep.sound.volume = 0.05;
  resources.footstep.sound.play();

  resources.ambient.sound.volume = 1;
  resources.ambient.sound.loop = true;
  resources.ambient.sound.play();
});
...
foxRunSprite.onLoop = () => {
  PIXI.Loader.shared.resources.footstep.sound.play();
};
...
function handleMove(ev) {
  ...
  if (
    ...
    if (key === "ArrowRight" || key === "d") {
      direction = "right";
      foxRunSprite.animationSpeed = 0.35;
      // foxJumpSprite.animationSpeed = .3;
    }
    if (key === "a" || key === "ArrowLeft") {
      direction = "left";
      foxRunSprite.animationSpeed = 0.2;
...
app.ticker.add(() => {
  if (moving) {
    if (
      direction === "right" &&
      foxRunSprite.position.x < window.innerWidth - foxRunSprite.width / 2
    ) {
    ...
    if (
      direction === "left" &&
      foxRunSprite.position.x > 0 + foxRunSprite.width / 2
    ) {
      foxRunSprite.position.x = foxRunSprite.position.x - 1;
      foxJumpSprite.position.x = foxRunSprite.position.x - 1;
    }
